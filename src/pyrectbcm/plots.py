...
def amplitude_plot(Input, ax = None, silent = None):
    ...
    if silent == None:
        ...
        ax.plot((xmin, xmin, xmax, xmax, xmin), (ymin, ymax, ymax, ymin, ymin), 1, color = 'k')
        plt.show()
    return True, ax
...
def geometry_plot(Input, t, ax = None):
    ...
    dims = np.array([0, (Basin.length+offset)*2, 0, (Basin.width+offset*2)])/1e3
    qs = 20
    if ax == None:
        fig = plt.figure()
        ax = Axes3D(fig)
    ...
    # Draw the sea
    sea = patches.Rectangle((0, dims[1]/2 + Inlets.lengths[0]/1e3), dims[3], dims[1]/2 - Inlets.lengths[0]/1e3, color = cs)
    ax.add_patch(sea)
    art3d.pathpatch_2d_to_3d(sea, z = 0.1)
    ax.plot((0, dims[3]), (dims[1]/2 + Inlets.lengths[0]/1e3, dims[1]/2 + Inlets.lengths[0]/1e3), 0.1, color = cc)

    # Draw the basin
    basin = patches.Rectangle((offset/1e3, offset/1e3), Basin.width/1e3, Basin.length/1e3, facecolor = cb, edgecolor = cc)
    ax.add_patch(basin)
    ...
    for inlet in range(Basin.numinlets):
        if Inlets.wit[t, inlet] > 0:
            ...
            inlet_ = patches.Rectangle((xi, dims[1]/2 - 0.1), Inlets.wit[t, inlet]/1e3, Inlets.lengths[inlet]*1e-3 + 0.1, facecolor = cs)
            ax.add_patch(inlet_)
            art3d.pathpatch_2d_to_3d(inlet_, z = 0.2)
            ax.plot((xi, xi), (dims[1]/2, dims[1]/2+Inlets.lengths[inlet]/1e3), 0.2, color = cc)
            ax.plot((xi+Inlets.widths[:, inlet]/1e3, xi+Inlets.widths[:, inlet]/1e3), (dims[1]/2, dims[1]/2+Inlets.lengths[inlet]/1e3), 0.2, color = cc)

    ax.set_aspect('equal')
    ax.axis('off')
    ax.plot((dims[2], dims[2], dims[3], dims[3], dims[2]), (dims[0], dims[1], dims[1], dims[0], dims[0]), 1, color = 'k')
    ax.grid(False)
    plt.show
    return True, ax
...
def evolution_plot_3p(Input, orientation, silent = None):
    ...
    if silent == None:
        ...
        ax2.set_xlim((-0.3*Basin.length)/1e3, (Basin.width+0.3*Basin.length)/1e3)
        ax2.set_aspect(ax1.get_ylim()[1]/ax2.get_ylim()[1]*1.1)
        geometry_plot(Input, Input.Inlets.wit.shape[0]-1, ax = ax3)
